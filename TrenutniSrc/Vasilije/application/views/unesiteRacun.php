<html>
	<head>
		<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css">
	</head>
	<body>
		<header>
			<h1 align="center"><img src="logo.png" alt="Logo" style="width:200px;"></h1>
		</header>
		
		 <nav class="navbar navbar-expand-md bg-dark navbar-dark">
			<a class="navbar-brand" href="pocetna.html">
				<img src="logo2.png" alt="logo" style="width:40px;">
			</a>
			<ul class="navbar-nav">
				<li class="nav-item">
					<a class="nav-link" href="pocetna.html">Početna</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" href="prijava.html">Prijava</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" href="ulogeVlasnika.html">Meni</a>
				</li>
			</ul>
		 </nav>
		 <br/> <a href="oglasnaTabla.html">
			<img src="ot.png" alt="tabla"style="width:60px;" align="left">
		</a>
		<a href="<?php echo site_url("Stanodavac/logout"); ?>">
			<img src="logout.png" alt="odjava"style="width:50px;" align="right">
		</a>
		<a href="profil.html">
			<img src="profil.png" alt="profil"style="width:45px;" align="right">
		</a>
		<br><br><br>
		 <h4 align="center">Unesite račun za podstanara:</h4>
		 <br/>
		 <table align='center' width='40%' bgcolor='black'>
			<tr bgcolor='ffaa80' height="50px">
				<td colspan='2'>&nbsp;</td>
			</tr>
			<tr height="350px">
				<td colspan='2' bgcolor='white' align='center' width='50%'>
					<br/>
					<h3>Napišite račun:</h3></h3>
					<br/>
                                        <form name="UnesiRacun" action="<?php  echo site_url('Stanodavac/unesiRacun')?>" method="post">
						<table>
							<tr>
								<td>Odaberite podstanara:</td>
								<td>
									<select>
                                                                            <?php 
                                                                            foreach ($result as $key):
                                                                            ?>
										<option ><?php echo $key->Ime;?></option>
                                                                                <?php endforeach;?>
									</select>
								</td>
							</tr>
							
							<tr align='center'>		
								<td colspan='2'>
								<br/>
								<hr/>
								<br/>
								</td>
							</tr>
							
							<tr>
								<td>Svrha uplate:</td>
								<td><input type='text' name='svrhaUplate'></td>
							</tr>
							
							<tr>
								<td>Poziv na broj:</td>
								<td><input type='text' name='pozivNaBroj'></td>
							</tr>
							
							<tr>
								<td>Broj računa za uplatu:</td>
								<td><input type='text' name='brRacuna'></td>
							</tr>
							
							<tr>
								<td>Iznos računa u dinarima:</td>
								<td><input type='number' name='iznosRacuna'></td>
							</tr>

							<tr align='center'>		
								<td colspan='2'>
								<hr/>
								<br/>
								<input type="submit" class="btn btn-success" value='Pošaljite račun'>
								</td>
							</tr>
						</table>
					</form>	
				
				</td>
				
			</tr>
			<tr bgcolor='#ffaa80' height="50px">
				<td colspan='2'>&nbsp;</td>
			</tr>
		</table>
		 <hr>
			<footer>
				<p><i>Copyright 2019, Strašan tim,
				Odsek za softversko inženjerstvo Elektrotehničkog fakulteta Univerziteta u Beogradu</i></p>
			</footer>
	</body>
</html>
